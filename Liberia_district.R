...
load("LB.population.Rdata")
Liberia_weekly_cases_climate <- left_join(Liberia_weekly_cases_climate, LB.population, by='Location')
dist_coordinates <- read.xlsx('geolocation_districts.xlsx',1) %>% select(-Country)
Liberia_weekly_cases_climate <- left_join(Liberia_weekly_cases_climate, dist_coordinates, by='Location')
