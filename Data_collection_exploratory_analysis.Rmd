...
As we did beforfe we explore the relationship between climate factors and incidence, but this time by main district. Again, we see the same pattern as described at the country level: temperature drops and the rainy season starts weeks before the onset of the outbreak.
...
We saw before that not all districts were affected. Thus we explore the relationship between the climatic factors and the incidence on all regions. 

COMPARE MAPS OF CLIMATE FACTORS AND INCIDENCE
Also diference in temperature over the rpevious weeks and incidence
Increase of rainfall over the rpevious weeks and incidence
...
tmp <- ggplot(three_countries_map_climate %>% 
    ...
    geom_polygon(aes(x = long, y = lat, group = group, color="Sierra Leone"), fill=NA, size = 0.75, data=SLshp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Liberia"), fill=NA, size = 0.75, data=LBshp.adm0) +
    coord_map() + scale_color_discrete(name="Country")+ facet_grid(Weeks~.) + theme_classic() +
...
ggplot(three_countries_map_climate %>% 
    ...
    geom_polygon(color = "black", size = 0.25) + scale_fill_gradient(low='yellow', high='red',name='Max Temperature\nin degrees Celsius') +
    geom_polygon(aes(x = long, y = lat, group = group, color="Guinea"), fill=NA, size = 0.75, data=guineashp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Sierra Leone"), fill=NA, size = 0.75, data=SLshp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Liberia"), fill=NA, size = 0.75, data=LBshp.adm0) +
    coord_map() + scale_color_discrete(name="Country")+ facet_grid(Weeks~.) + theme_classic() +
    ggtitle('Max Temperatures per region, Year 2014')

#Min Temp
ggplot(three_countries_map_climate %>% 
           filter(count_week==77 | count_week==89 | count_week==101|count_week==113)
       , aes(x = long, y = lat, group = group, fill = tmn)) +
    geom_polygon(color = "black", size = 0.25) + scale_fill_gradient(low='yellow', high='red',name='Min Temperature\nin degrees Celsius') +
    geom_polygon(aes(x = long, y = lat, group = group, color="Guinea"), fill=NA, size = 0.75, data=guineashp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Sierra Leone"), fill=NA, size = 0.75, data=SLshp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Liberia"), fill=NA, size = 0.75, data=LBshp.adm0) +
    coord_map() + scale_color_discrete(name="Country")+ facet_grid(Weeks~.) + theme_classic() +
    ggtitle('Min Temperatures per region, Year 2014')
...
ggplot(three_countries_map_climate %>% 
    ...
    geom_polygon(color = "black", size = 0.25) + scale_fill_gradient(low='green', high='yellow',name='Incidence per week') +
    geom_polygon(aes(x = long, y = lat, group = group, color="Guinea"), fill=NA, size = 0.75, data=guineashp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Sierra Leone"), fill=NA, size = 0.75, data=SLshp.adm0) +
    geom_polygon(aes(x = long, y = lat, group = group, color="Liberia"), fill=NA, size = 0.75, data=LBshp.adm0) +
    coord_map() + scale_color_discrete(name="Country")+ facet_wrap(Weeks~.) + theme_classic()+
    ggtitle('Weekly incidence per region, Year 2014')
```
